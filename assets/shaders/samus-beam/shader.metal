...
@end
    ...
    float time = float(frameUniforms.frameCount) * 0.05;

    float wave1 = sin(in.position.x * 0.04 + time * 1.2) * cos(in.position.y * 0.04 - time);
    float wave2 = sin(in.position.x * 0.07 - time * 2.0) * cos(in.position.y * 0.03 + time * 1.5);
    ...
    float3 etherRed = float3(1.5, 0.05, 0.25);
    float3 coreWhite = float3(2.0, 1.8, 1.9);

    float3 finalEnergy = float3(0.2, 0.0, 0.05);
    finalEnergy += etherRed * aura * 1.5;
    finalEnergy += coreWhite * core * 2.5 * pulse;

    texel.xyz = finalEnergy;
