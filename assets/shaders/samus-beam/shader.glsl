...
@else
    ...
    void main() {
        ...
        float time = float(frame_count) * 0.05;

        float wave1 = sin(gl_FragCoord.x * 0.04 + time * 1.2) * cos(gl_FragCoord.y * 0.04 - time);
        float wave2 = sin(gl_FragCoord.x * 0.07 - time * 2.0) * cos(gl_FragCoord.y * 0.03 + time * 1.5);

        float plasma = (wave1 + wave2) * 0.5;
        plasma = plasma * 0.5 + 0.5;

        float aura = smoothstep(0.3, 0.8, plasma);
        float core = pow(plasma, 5.0);

        float pulse = (sin(time * 6.0) * 0.5 + 0.5) * 0.4 + 0.6;
        vec3 etherRed = vec3(1.5, 0.05, 0.25);
        vec3 coreWhite = vec3(2.0, 1.8, 1.9);

        vec3 finalEnergy = vec3(0.2, 0.0, 0.05);
        finalEnergy += etherRed * aura * 1.5;
        finalEnergy += coreWhite * core * 2.5 * pulse;

        @if(o_alpha)
            texel.rgb = clamp(finalEnergy, 0.0, 1.0);
        @else
            texel = clamp(finalEnergy, 0.0, 1.0);
        @end
