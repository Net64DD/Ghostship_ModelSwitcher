...
float4 PSMain(PSInput input, float4 screenSpace : SV_Position) : SV_TARGET {
    ...
    float time = (float)noise_frame * 0.05f;

    float wave1 = sin(screenSpace.x * 0.04f + time * 1.2f) * cos(screenSpace.y * 0.04f - time);
    float wave2 = sin(screenSpace.x * 0.07f - time * 2.0f) * cos(screenSpace.y * 0.03f + time * 1.5f);

    float plasma = (wave1 + wave2) * 0.5f;
    plasma = plasma * 0.5f + 0.5f;

    float aura = smoothstep(0.3f, 0.8f, plasma);
    float core = pow(plasma, 5.0f);

    float pulse = (sin(time * 6.0f) * 0.5f + 0.5f) * 0.4f + 0.6f;
    float3 etherRed = float3(1.5f, 0.05f, 0.25f);
    float3 coreWhite = float3(2.0f, 1.8f, 1.9f);

    float3 finalEnergy = float3(0.2f, 0.0f, 0.05f);
    finalEnergy += etherRed * aura * 1.5f;
    finalEnergy += coreWhite * core * 2.5f * pulse;

    @if(o_alpha)
        texel.rgb = saturate(finalEnergy);
    @else
        texel = saturate(finalEnergy);
    @end
